...
// ======================================================================
#define RH_RXD 5   // Right hand RX
#define RH_TXD 6  // Right hand TX
#define LH_RXD 17  // Left hand RX
#define LH_TXD 18  // Left hand TX
#define HD_RXD 7  // Head RX
#define HD_TXD 15  // Head TX
#define BMS_RXD 38 // BMS RX
#define BMS_TXD 37 // BMS TX
#define MOTOR_TX 1 // TX pin for Serial1
#define MOTOR_RX 2  // RX pin for Serial1
...
class ControlCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *pCharacteristic) override {
    ...
    if (rxValue.length() > 0) {
      ...
      if (doc.containsKey("motors")) {
        ...
        if (motors.containsKey("m2")) {
          ...
          motor2.setSpeed(m2);
        	...
        	dc.ddsm_ctrl(1 ,m1, 2); // 50 rpm
         	dc.ddsm_ctrl(2 ,-m2, 2); // 50 rpm
          dc.ddsm_ctrl(1 ,m1, 2); // 50 rpm
         	dc.ddsm_ctrl(2 ,-m2, 2); // 50 rpm
          dc.ddsm_ctrl(1 ,m1, 2); // 50 rpm
         	dc.ddsm_ctrl(2 ,-m2, 2); // 50 rpm
      ...
      {
        JsonObject charge = doc["charge"];
        if(charge.containsKey("status"))
        {
    ...
    if (status.length() > 0) {
      ...
      // Parse the command (e.g., "ON" or "OFF")
      if (status == "true") {
...
void setup() {
  ...
  SerialMOTOR.begin(DDSM_BAUDRATE, SERIAL_8N1, MOTOR_RX, MOTOR_TX);
